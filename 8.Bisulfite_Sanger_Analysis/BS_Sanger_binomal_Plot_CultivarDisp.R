...
Loci1_Acc.Lst <- c("C13", "L29", "L36", "Q14", "Q24", "Q28")


Loci1.Dat <- lapply(Loci1_Acc.Lst, function(acc) {
  file_path <- paste0("Disp_Cultivar_OSH3/hyperTE_OSH3_", acc, ".csv")
...
Loci2_Acc.Lst <- c("C13", "L6", "L36", "Q18", "Q24", "Q28")

Loci2.Dat <- lapply(Loci2_Acc.Lst, function(acc) {
  file_path <- paste0("Disp_Cultivar_SH6/hyperTE_SH6_", acc, ".csv")
...
Loci3_Acc.Lst <- c("L6", "L32", "L36", "Q18", "Q28", "Q37")


Loci3.Dat <- lapply(Loci3_Acc.Lst, function(acc) {
  file_path <- paste0("Disp_Cultivar_Loci3/hyperTE_Loci3_", acc, ".csv")
...
write_tsv(filter(Dat, site.type=="Overall"), "Fig1h.Cultivar.txt")
...
#ggsave("Fig/DispCultivar.mC_C.pdf", width = 2.7, height = 2.2)
...
filter(Dat, site.type=="Overall", Pop=="Cultivar", site.type=="Overall")$methylation_rate
filter(Dat, site.type=="Overall", Pop=="Wild", site.type=="Overall")$methylation_rate
...
#ggsave("Fig/DispCultivar.mC_C.Context.pdf", width = 4, height = 2)
