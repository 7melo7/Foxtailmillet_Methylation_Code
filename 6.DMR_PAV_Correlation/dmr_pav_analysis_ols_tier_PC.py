#!/usr/bin/env python3
"""
DMR-PAV Linear Regression with Tiered Permutation (OLS, +PC)
=============================================================
Linear model per DMR-PAV pair:
  DMR = α + β₁·PC1 + ... + β₅·PC5 + γ·G_p + ε
...
Permutation comparison: perm_tier < real_tier → counts as "beat".
Same-tier noise is ignored.

Advantage over logistic: OLS is 10-20× faster (closed-form, no IRLS iteration).
The permutation framework corrects for any distributional violations.
"""
...
def load_data(data_dir, n_pcs=5, pairwise_file="DMR_overlap_PAV.20kbp.pairwise"):
    ...
    pca_rows, pca_samples = [], []
    with open(os.path.join(data_dir, "PCA.Setaria_syri_60.eigenvec"), "r") as f:
        for line in f:
            parts = line.strip().split()
            pca_samples.append(parts[0])
            pca_rows.append([float(x) for x in parts[2:2 + n_pcs]])
    pca_full = np.array(pca_rows, dtype=np.float64)
    pca_reorder = [pca_samples.index(s) for s in canonical]
    pca = pca_full[pca_reorder, :]
    print(f"    {pca.shape[1]} PCs", flush=True)

    X_base = np.column_stack([np.ones(len(canonical), dtype=np.float64), pca])
...
def main():
    p = argparse.ArgumentParser(description="DMR-PAV OLS Tier Permutation (+PC)")
    p.add_argument("--data-dir", default=".")
    p.add_argument("--pairwise-file", default="DMR_overlap_PAV.20kbp.pairwise",
                   help="Pairwise input filename (inside data-dir)")
    p.add_argument("--output-dir", default="./output_ols_tier")
    p.add_argument("--n-permutations", type=int, default=N_PERMUTATIONS)
    p.add_argument("--n-threads", type=int, default=N_THREADS)
    p.add_argument("--n-pcs", type=int, default=N_PCS)
    ...
    print(f"OLS tier (+PC)  |  Perms: {args.n_permutations}  |  "
          f"Threads: {args.n_threads}  |  PCs: {args.n_pcs}")

    # ── Phase 1 ──
    with timer("Phase 1 – Load"):
        (dmr_mat, pav_mat, X_base, dmr_ids, pav_ids,
         _, _, dmr_pav_pairs) = load_data(args.data_dir, args.n_pcs, args.pairwise_file)
